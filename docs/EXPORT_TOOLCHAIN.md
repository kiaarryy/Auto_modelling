...
The HX export helper uses Dymola's bundled Python interface by default:

```powershell
python scripts\legacy_site_a\export_site_a_hx_fmus.py --run
```

Use `--batch` only to audit the legacy `/nowindow` MOS invocation.
